...
front-end, the SPI-NOR flash, the vocoder, the display, and a handful of analog
...
for DMR), **RDA5802E** broadcast-FM tuner (the "FM radio" feature), a footprint for an
**HR_V3000** AMBE+2 vocoder — but note that on this hardware DMR voice is done by
a *software* AMBE+2 codec on the CK803S and the vocoder nets are unpopulated (see
[voice codecs](07-voice-codecs)) — and **ST7735S** 160×128 color
...
| `0x14150000`  | SPI Master 3 = V_SPI, wired to the **HR_V3000 footprint** (status `+0x28`, data `+0x60`) — footprint **unpopulated** on this board, so `vspi_*` target nothing (see [voice codecs](07-voice-codecs)) | APB |
| `0x15000000`  | SDIO                                                | AHB  |
...
- **HR_V3000** — an **unpopulated footprint** on this board: the V_SPI (SPI_M3, `0x14150000`) nets and the internal I2S/PCM link into the modem are laid out but the device is not placed, so the CPU's `vspi_read_reg`/`vspi_write_reg` target nothing here. Don't try to program a live vocoder over this path — DMR voice is a *software* AMBE+2 codec on the CK803S (see [voice codecs](07-voice-codecs)).
...
- ⚠️ **SPI Master 3 (`0x14150000`) is V_SPI to the vocoder, NOT "EFUSE only."** The older `docs/peripherals.md` note calling it EFUSE-dedicated is superseded by `docs/device_subsystem.md`.
- ⚠️ **The vocoder's I2S clocks and HR_V3000 enable (manual reg `0x06=0x24`, dividers `0x2F..0x33`) are never written by CK803S firmware.** They come up from the modem boot ROM or a silicon strap. Don't hunt the CPU app for that init — it isn't there.
...
to hardware controllers and verified on-device). The internal vocoder/I2S plumbing
inside the C7000 die remains 🟡 (opaque; behind the modem boot ROM).
...
- `docs/device_subsystem.md` — V_SPI/vocoder wiring, I2S-init absence, AES path.
