...
- ⚠️ **Dead end — register-value replication from a one-shot diag op does NOT start the engine.** An earlier session (memory `hd2-dmr-tx-progress`) exhaustively replicated the vendor's full keyed end-state (config + 0x400 sequence + `WORK_MODE=0x6e` + sync TDMA enables + clocks) and still got `bursts=0`. The working path is the temp-slot arm sequence above, run inside the driver — not a snapshot replay.
